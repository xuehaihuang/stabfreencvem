...
\XH{A stabilization term is usually required in the virtual element methods (VEMs) to ensure the coercivity of the discrete bilinear form \cite{BeiraoBrezziCangianiManziniEtAl2013,BeiraoBrezziMariniRusso2014}, while the stabilization term also brings about some problems. The local stabilization term $S_K(\cdot, \cdot)$ has to satisfy 
$$
...
Recently a stabilization-free linear VEM, based on a higher order polynomial projection of the gradient of virtual element functions, is devised for the Poisson equation in two dimensions in \cite{BerroneBorioMarcon2021}, where the degree of polynomial used in the projection depends on the number of vertices of the polygon \XH{and generally the geometry of the polygon}. 
\XH{Numerical examples in \cite{BerroneBorioMarcon2022} show that the stabilization-free VEM in \cite{BerroneBorioMarcon2021} outperforms the standard VEM in \cite{BeiraodaVeigaBrezziMariniRusso2016} for anisotropic elliptic problems on general convex polygonal meshes.}
The stabilization-free VEM in \cite{BerroneBorioMarcon2021} is then applied to solve the Laplacian eigenvalue problem \cite{MengWangBuMei2022}. Along this line, similar stabilization-free virtual element methods are devised for a plane elasticity problem in~\cite{DAltriMirandaPatrunoSacco2021,ChenSukumar2022,ChenSukumar2023}. \XH{By the way, we refer to \cite{CicuttinErnLemaire2019} for a stabilization-free hybrid high-order method, and 
...
This motivates us to construct stabilization-free VEMs in arbitrary degree of polynomial, including a nonconforming VEM in arbitrary dimension and a conforming VEM in two dimensions,  in a unified way.
}

The key to construct stabilization-free VEMs is to find a finite-dimensional space $\mathbb{V}(K)$ for polytope $K$ and a projector $Q_K$ onto the space $\mathbb{V}(K)$ such that
\begin{enumerate}[(C1)]
...
\end{enumerate}
\XH{The hidden constants in \eqref{intro:gradVknormequiv} are independent of the size of $K$, but depend on the degree of polynomials, and the chunkiness parameter and the geometric dimension of $K$; see Section~\ref{sec:meshcondition} for details.}
...
Instead, \XH{based on a regular simplicial tessellation of polytope $K$}, we employ $k$-th order or $(k-1)$-th order $H(\div)$-conforming macro finite elements as $\mathbb{V}(K)$ in this paper, and keep the virtual element space $V_k(K)$ as the usual ones.
...
By the aid of the projector $Q_{K,k}^{\div}$, we advance a stabilization-free nonconforming VEM in arbitrary dimension and a stabilization-free conforming VEM in two dimensions  for second order elliptic problems.
Indeed, these stabilization-free VEMs can be equivalently recast as
primal mixed VEMs.
We prove the norm equivalence \eqref{intro:gradVknormequiv} and the well-posedness of these stabilization-free VEMs, and derive the optimal error estimates. Numerical experiments are provided to test these VEMs.

The idea on constructing stabilization-free VEMs in this paper is simple,
...
In Section~\ref{sec:divmacrofem}, $H(\div)$-conforming macro finite elements in arbitrary dimension are constructed. A stabilization-free nonconforming VEM in arbitrary dimension is developed in Section \ref{sec:stabfreencfmvem}, and a stabilization-free conforming VEM in two dimensions is devised in Section \ref{sec:stabfreecfmvem}.
...
\end{align}
for $T\in\mathcal T_K$. Here $\mathbb P_{k-2}(T)/\mathbb R:=\mathbb P_{k-2}(T)\cap L_0^2(T)$, \XH{and $\mathbb P_{k-3}(T;\mathbb K)\boldsymbol{x}:=\{\boldsymbol{\tau}\boldsymbol{x}: \boldsymbol{\tau}\in \mathbb P_{k-3}(T;\mathbb K)\}$ with $\boldsymbol{x}\in T$ being the independent variable}.
...
\XH{Recall that \cite[(35)]{Chen;Huang:2020Finite}
...
\end{equation*}
we acquire from $\boldsymbol{w}\cdot\boldsymbol{x}=0$ that $(I+\boldsymbol{x}\cdot\nabla)\boldsymbol{w}=\boldsymbol{0}$, which \XH{together with \eqref{eq:20230205}} implies $\boldsymbol{w}=\boldsymbol{0}$.
\end{proof}
...
\end{lemma}
\begin{proof}
\XH{By the inverse inequality \cite{Ciarlet1978,Verfuerth2013} (see also \cite[Lemma 10]{Huang2020}),
$$
...
Next we focus on the proof of the \XH{lower bound}. Again we only prove the case $k\geq2$, whose argument can be applied to case $k=1$. Take $\boldsymbol{\phi}_1\in\boldsymbol{V}_{k-1}^{\mathrm{BDM}}(K)$ such that $(\boldsymbol{\phi}_1\cdot\boldsymbol{n})|_{\partial K}=(\boldsymbol{\phi}\cdot\boldsymbol{n})|_{\partial K}$, and \XH{all the DoFs \eqref{BDMdof1}-\eqref{BDMdof3} of $\boldsymbol{\phi}_1$ interior to $K$ equal to zero}. 
\XH{By the norm equivalence on each simplex $T$ and the vanishing DoFs \eqref{BDMdof2}-\eqref{BDMdof3}, we get
...
\]
Obviously \XH{we obtain from \eqref{eq:202302011} that}
...
\end{equation}
\XH{Let $I_K^{\div}: \boldsymbol{H}_0(\div,K)\to\mathring{\boldsymbol{V}}_{k-1}^{\mathrm{BDM}}(K)$ be the local $L^2$-bounded commuting projection operator in \cite{ArnoldGuzman2021,FalkWinther2014}, then
...
$.
} Set $\boldsymbol{\phi}_2=-I_K^{\div}(\nabla w)\in\mathring{\boldsymbol{V}}_{k-1}^{\mathrm{BDM}}(K)$. \XH{We have
...
\]
\XH{On each face $F\in\mathcal F(K)$, $(\boldsymbol{\phi}\cdot\boldsymbol{n})|_F$ is a polynomial for $\boldsymbol{\phi}\in\mathbb{V}_{k-1}^{\rm div}(K)$ but $(\boldsymbol{\phi}\cdot\boldsymbol{n})|_F$ is a piecewise polynomial for $\boldsymbol{\phi}\in\boldsymbol{V}_{k-1}^{\rm div}(K)$.}
...
In this section we will develop a stabilization-free nonconforming VEM for the second order elliptic problem in arbitrary dimension
...
\XH{Several $H^1$-nonconforming virtual elements have been developed in \cite{AyusodeDiosLipnikovManzini2016,CangianiManziniSutton2017,ChenHuang2020ncvem,Huang2020}.
...
\end{align*}
which combined with \XH{both $H^1$-$L^2$ and $L^2$ boundary-$L^2$ bulk inverse inequalities} for polynomials implies
...
\]
Thanks to the Poincar\'e-Friedrichs inequality \XH{\cite{Necas1967}} %\cite[(2.15)]{BrennerSung2018} 
...
\end{align*}
Then $(\boldsymbol{\phi}\cdot\boldsymbol{n})|_{F}=h_K^{-1}Q_{k-1}^Fv$ for $F\in\mathcal F(K)$. Since $\div\boldsymbol{\phi}\in \mathbb P_{\max\{k-2,0\}}(K)$, we have $\div\boldsymbol{\phi}-Q_{0}^K(\div\boldsymbol{\phi})=-h_K^{-2}Q_{k-2}^Kv$. Apply the integration by parts \XH{and the fact $v=v-Q_{0}^Kv\in L_0^2(K)$} to get
\begin{align*}
...
\end{align*}
\XH{Noting that $\div\boldsymbol{\phi}-Q_{0}^K(\div\boldsymbol{\phi})=-h_K^{-2}Q_{k-2}^Kv$ and $(\boldsymbol{\phi}\cdot\boldsymbol{n})|_{F}=h_K^{-1}Q_{k-1}^Fv$ for $F\in\mathcal F(K)$, we have
\[
...
\end{align*}
We have the discrete Poincar\'e inequality \XH{\cite{Brenner2003}}%(cf. \cite[(4.16)]{ChenHuang2020ncvem})
\begin{equation}\label{eq:vempoincareineqlty}
\|v_h\|_0\lesssim |v_h|_{1,h} \quad\forall~v_h\in V_h.
\end{equation}
\XH{Hence $|\cdot|_{1,h}$ is indeed a norm for $V_h$.}
...
\XH{
\begin{remark}\rm
The stabilization-free mixed-order HHO method for the Poisson equation in~\cite{CicuttinErnLemaire2019} is equivalent to find $u_h\in W_h$ such that
...
Let $u\in H_0^1(\Omega)$ be the solution of problem \eqref{eq:ellipitc2ndproblem}, and $u_h\in V_h$ be the solution of the VEM \eqref{eq:vem}. Assume $u\in H^{k+1}(\Omega)$ and $f\in H^{k-1}(\Omega)$. Then
...
In this section we will develop a stabilization-free conforming VEM for the second order elliptic problem \eqref{eq:ellipitc2ndproblem} in two dimensions.
% \begin{equation}\label{eq:ellipitc2ndproblem}
% \begin{cases}
% -\Delta u + \alpha u=f & \textrm{ in } \Omega,\\
% \qquad\qquad u=0&\textrm{ on } \partial\Omega,
...
&\qquad\qquad\qquad\qquad\qquad\qquad\quad\;\;\; \textrm{and } (v-\Pi_k^Kv, q)_K=0\quad\forall~q\in\XH{\mathbb P_{k-2}^{\perp}(K)}\},
\end{align*}
...
\]
The stabilization-free VEM \eqref{eq:cfmvem} is uni-solvent.
...
\XH{In this section, we will numerically test the stabilization-free nonconforming virtual element method \eqref{eq:vem} and the stabilization-free conforming
...
\XH{The only difference between the standard VEMs and the stabilization-free VEMs is the
...
\XH{Secondly, we do the patch test for the Laplace equation, i.e. problem \eqref{eq:ellipitc2ndproblem} with $\alpha = 0$ and $f=0$, but the Dirichlet boundary condition is nonhomogeneous. Take the exact solution $u=1+x+y$. %We adopt meshes in Fig. \ref{fig:polymesh}.
% $$
% \left\{\begin{aligned}
