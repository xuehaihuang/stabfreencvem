...
Recently a stabilization-free linear VEM, based on a higher order polynomial projection of the gradient of virtual element functions, is devised for the Poisson equation in two dimensions in \cite{BerroneBorioMarcon2021}, where the degree of polynomial used in projection depends on the number of vertices of the polygon \XH{and generally the geometry of the polygon}. 
...
On the face $F\in\mathcal F_h$, define surface divergence
\[
...
\]
For a smooth function $v$, define surface gradient $\nabla_Fv:=\nabla v-(\partial_nv)\boldsymbol{n}$.
...
\end{align}
for $T\in\mathcal T_K$. Here $\mathbb P_{k-2}(T)/\mathbb R:=\mathbb P_{k-2}(T)\cap L_0^2(T)$.
...
% for $T\in\mathcal T_K$.
Now let $w\in H^1(K)/\mathbb R$ be the solution of 
\[
\begin{cases}
...
\XH{Several $H^1$-nonconforming virtual elements are developed in \cite{AyusodeDiosLipnikovManzini2016,CangianiManziniSutton2017,ChenHuang2020ncvem,Huang2020}.
...
\begin{theorem}
The stabilization free virtual element method \eqref{eq:vem} is \XH{well-posed}.
\end{theorem}
\begin{proof}
...
For $v\in V_k(K)$, the $H^1$ projection $\Pi_k^Kv$ and the $L^2$ projection $Q_k^Kv= \Pi_k^Kv + Q_{k-2}^Kv-Q_{k-2}^K\Pi_k^Kv$ are computable using only the DoFs \eqref{eq:cfmvemdof1}-\eqref{eq:cfmvemdof3}. 
...
In this section, we will numerically test the stabilization-free nonconforming virtual element method \eqref{eq:vem} and the stabilization-free conforming
...
In both SFNCVEM and SFCVEM, we choose $k = 1, 2, 5$.
