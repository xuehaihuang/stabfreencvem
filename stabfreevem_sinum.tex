...
We intend to construct VEMs without extrinsic stabilization in arbitrary degree of polynomial, \XH{including a nonconforming VEM and a conforming VEM in arbitrary dimension under the mesh assumption that all the faces of each polytope are simplices}, in a unified way.
...
these considerations and the direct decomposition of the $H(\div)$-conforming
macro finite element space $\mathbb{V}_{k}^{\rm div}(K)$, we propose
...
By the aid of the projector $Q_{K,k}^{\div}$, \XH{we advance a nonconforming VEM and a conforming VEM without extrinsic stabilization for second order elliptic problems in arbitrary dimension.}
...
Given a $d$-dimensional polytope $K$, \XH{let $\Delta_j(K)$ be the set of all $j$-dimensional faces of $K$ for $j=0,1,\ldots, d-1$. Set $\mathcal{F}(K):=\Delta_{d-1}(K)$ and $\mathcal{E}(K):=\Delta_{d-2}(K)$.}
% let $\mathcal{F}(K)$ and $\mathcal{E}(K)$ be the set of all $(d-1)$-dimensional faces and $(d-2)$-dimensional faces of $K$ respectively.
...
\begin{itemize}
 \item[(A1)] Each element $K\in \mathcal T_h$ and each face $F\in \mathcal F_h^r$ for $1\leq r\leq d-1$ is star-shaped with a uniformly bounded chunkiness parameter. \XH{All the $(d-1)$-dimensional faces of $K$ are simplices.}

 \item[(A2)] There exists a shape-regular simplicial mesh $\mathcal T_h^*$ such that each $K\in \mathcal T_h$ is a union of some simplexes in $\mathcal T_h^*$. For polytope $K\in \mathcal T_h$, let $\mathcal T_K$ be the simplicial partition of $K$ induced from $\mathcal T_h^*$. Assume simplicial partition $\mathcal T_K$ is quasi-uniform, \XH{and all the $(d-1)$-dimensional faces of $K$ are also faces of $\mathcal T_K$.}
\end{itemize}
...
% For polytope $K\in \mathcal T_h$, denote by $\mathcal T_K$ the simplicial partition of $K$, which is induced from $\mathcal T_h^*$. 
% Let $\mathcal{F}(\mathcal T_K)$ and $\mathcal{E}(\mathcal T_K)$ be the set of all $(d-1)$-dimensional faces and $(d-2)$-dimensional faces of the simplicial partition $\mathcal T_K$ respectively. 
% Set 
% \[
...
\end{equation}
% {where $\mathbb P_{k-2}(T;\mathbb K)\boldsymbol{x}:=\{\boldsymbol{\tau}\boldsymbol{x}: \boldsymbol{\tau}\in \mathbb P_{k-2}(T;\mathbb K)\}$ with $\boldsymbol{x}\in T$ being the independent variable.}
...
\dim\div\skw\boldsymbol{V}_{k}^{d-2}(K)-\dim\div\skw\mathring{\boldsymbol{V}}_{k}^{d-2}(K)={k+d-2\choose d-1}\#\XH{\mathcal F(K)}-1.
\end{equation}
% \begin{align*}  
...
\[
\XH{\mathbb{V}_{k-1}^{\rm div}(K)}:=\{\boldsymbol{\phi}\in\boldsymbol{V}_{k-1}^{\mathrm{BDM}}(K): \div\boldsymbol{\phi}\in\mathbb P_{k-2}(K)\},
\]
for $k\geq 2$, and 
\[
\XH{\mathbb{V}_{0}^{\rm div}(K)}:=\{\boldsymbol{\phi}\in\boldsymbol{V}_{0}^{\mathrm{RT}}(K): \div\boldsymbol{\phi}\in\mathbb P_{0}(K)\}.
\]
Apparently $\mathbb P_{k-1}(K;\mathbb R^d)\subseteq\XH{\mathbb{V}_{k-1}^{\rm div}(K)}$, $\XH{\mathbb{V}_{0}^{\rm div}(K)}\cap\ker(\div)=\boldsymbol{V}_0^{\mathrm{RT}}(K)\cap\ker(\div)$, and $\XH{\mathbb{V}_{k-1}^{\rm div}(K)}\cap\ker(\div)=\boldsymbol{V}_{k-1}^{\mathrm{BDM}}(K)\cap\ker(\div)$ for $k\geq2$.


In the following lemma we present a direct sum decomposition of space $\XH{\mathbb{V}_{k-1}^{\rm div}(K)}$.

\begin{lemma}
For $k\geq1$,
it holds
\begin{equation}\label{eq:Vdivlocaldecomp}
\XH{\mathbb{V}_{k-1}^{\rm div}(K)}=\div\skw \boldsymbol{V}_{k}^{d-2}(K) \oplus (\boldsymbol{x}-\boldsymbol{x}_K)\mathbb P_{\max\{k-2,0\}}(K).
\end{equation}
Then the complex
\begin{equation*}
\boldsymbol{V}_{k}^{d-2}(K)\xrightarrow{\div\skw}\mathbb{V}_{k-1}^{\rm div}(K)\xrightarrow{\div} \mathbb P_{\max\{k-2,0\}}(K)\to0    
\end{equation*}
is exact.
\end{lemma}
\begin{proof}
We only prove the case $k\geq2$, as the proof for case $k=1$ is similar.
Since $\div:(\boldsymbol{x}-\boldsymbol{x}_K)\mathbb P_{k-2}(K)\to\mathbb P_{k-2}(K)$ is bijective \cite[Lemma 3.1]{ChenHuang2021divdiv}, we have $\div\skw \boldsymbol{V}_{k}^{d-2}(K)\cap(\boldsymbol{x}-\boldsymbol{x}_K)\mathbb P_{k-2}(K)=\{\boldsymbol{0}\}$. Clearly $\div\skw \boldsymbol{V}_{k}^{d-2}(K) \oplus(\boldsymbol{x}-\boldsymbol{x}_K)\mathbb P_{k-2}(K)\subseteq \XH{\mathbb{V}_{k-1}^{\rm div}(K)}$.

On the other side, for $\boldsymbol{\phi}\in \XH{\mathbb{V}_{k-1}^{\rm div}(K)}$, by $\div\boldsymbol{\phi}\in \mathbb P_{k-2}(K)$, there exists a $q\in\mathbb P_{k-2}(K)$ such that $\div((\boldsymbol{x}-\boldsymbol{x}_K)q)=\div\boldsymbol{\phi}$, i.e. $\boldsymbol{\phi}-(\boldsymbol{x}-\boldsymbol{x}_K)q\in\XH{\mathbb{V}_{k-1}^{\rm div}(K)}\cap\ker(\div)=\boldsymbol{V}_{k-1}^{\mathrm{BDM}}(K)\cap\ker(\div)$. Thanks to finite element complex \eqref{eq:localBDMfemderhamcomplex}, $\boldsymbol{\phi}-(\boldsymbol{x}-\boldsymbol{x}_K)q\in\div\skw \boldsymbol{V}_{k}^{d-2}(K)$. Thus \eqref{eq:Vdivlocaldecomp} follows.
\end{proof}

Based on the space decomposition \eqref{eq:Vdivlocaldecomp} and the degrees of freedom of BDM element, we propose the following DoFs for space \XH{$\mathbb{V}_{k-1}^{\rm div}(K)$}
\begin{align}
    (\boldsymbol{\phi}\cdot\boldsymbol{n}, q)_F & \quad\forall~q\in\mathbb P_{k-1}(F) \textrm{ on each }  F\in\XH{\mathcal F(K)}, \label{Vdivdof1}\\
...
\begin{lemma}
The set of DoFs \eqref{Vdivdof1}-\eqref{Vdivdof3} is uni-solvent for space \XH{$\mathbb{V}_{k-1}^{\rm div}(K)$}.
\end{lemma}
\begin{proof}
By \eqref{eq:20220324-4} and \eqref{eq:Vdivlocaldecomp},
the number of DoFs \eqref{Vdivdof1}-\eqref{Vdivdof3} is
\begin{align*}
&\quad {k+d-2\choose d-1}\#\XH{\mathcal F(K)}+\dim\mathbb P_{\max\{k-2,0\}}(K)-1+\dim\div\skw\mathring{\boldsymbol{V}}_{k}^{d-2}(K) \\
&=\dim\div\skw\boldsymbol{V}_{k}^{d-2}(K)+\dim\mathbb P_{\max\{k-2,0\}}(K)=\dim\XH{\mathbb{V}_{k-1}^{\rm div}(K)}.
\end{align*}
% By \eqref{eq:Vdivlocaldecomp},
% the number of DoFs \eqref{Vdivdof1}-\eqref{Vdivdof3} equals to $\dim\boldsymbol{V}_{k-1}^{\rm div}(K)$.

Assume $\boldsymbol{\phi}\in\XH{\mathbb{V}_{k-1}^{\rm div}(K)}$ and all the DoFs \eqref{Vdivdof1}-\eqref{Vdivdof3} vanish. By the vanishing DoF~\eqref{Vdivdof1}, $\boldsymbol{\phi}\in \boldsymbol{H}_0(\div, K)$ and $\div\boldsymbol{\phi}\in L_0^2(K)$. Then it follows from the vanishing DoF~\eqref{Vdivdof2} that $\div\boldsymbol{\phi}=0$. 
...
Next we consider the norm equivalence of space \XH{$\mathbb{V}_{k-1}^{\rm div}(K)$}.


\begin{lemma}\label{lem:Vkm1divnormequivalence}
For $\boldsymbol{\phi}\in\XH{\mathbb{V}_{k-1}^{\rm div}(K)}$, it holds the norm equivalence
\begin{equation}\label{eq:Vkm1divnormequivalence}
\|\boldsymbol{\phi}\|_{0,K}\eqsim h_K\|\div\boldsymbol{\phi}\|_{0,K} + \sup_{\boldsymbol{\psi}\in\div\mathring{\boldsymbol{V}}_{k}^{d-2}(K)}\frac{(\boldsymbol{\phi}, \boldsymbol{\psi})_K}{\|\boldsymbol{\psi}\|_{0,K}} +\sum_{F\in\XH{\mathcal F(K)}}h_F^{1/2}\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}.
\end{equation}
\end{lemma}
\begin{proof}
...
\end{equation}
For $F\in\XH{\mathcal F(K)}$, there exists a simplex $T\in\mathcal T_K$ satisfying $F\subset\partial T$, then apply the trace inequality \cite[Theorem 1.5.1.10]{Grisvard1985} (see also \cite[(2.18)]{BrennerSung2018}) and the inverse inequality to get
$$
...
This means
$$
\sum_{F\in\XH{\mathcal F(K)}}h_F^{1/2}\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F} \lesssim \sum_{F\in\XH{\mathcal F(K)}}\|\boldsymbol{\phi}\|_{0,T}\lesssim \|\boldsymbol{\phi}\|_{0,K}.
$$ 
Combining \eqref{eq:20230201}, the Cauchy-Schwarz inequality and the last inequality yields
$$
h_K\|\div\boldsymbol{\phi}\|_{0,K}+\sup_{\boldsymbol{\psi}\in\div\mathring{\boldsymbol{V}}_{k}^{d-2}(K)}\frac{(\boldsymbol{\phi}, \boldsymbol{\psi})_K}{\|\boldsymbol{\psi}\|_{0,K}} +\sum_{F\in\XH{\mathcal F(K)}}h_F^{1/2}\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}\lesssim \|\boldsymbol{\phi}\|_{0,K}.
$$
% \begin{align*}
...
\|\boldsymbol{\phi}_1\|_{0,K}^2 = \sum_{T\in\mathcal T_K}\|\boldsymbol{\phi}_1\|_{0,T}^2 \eqsim \sum_{T\in\mathcal T_K}\sum_{F\in\mathcal F(T)}h_F\|\boldsymbol{\phi}_1\cdot\boldsymbol{n}\|_{0,F}^2 =\sum_{F\in\XH{\mathcal F(K)}}h_F\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}^2.
\end{align}
...
\leq\frac{1}{|T|}\sum_{F\in\mathcal F(T)\cap\XH{\mathcal F(K)}}|F|\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}^2\;\;\forall~T\in\mathcal T_K.
\end{equation}
% for $T\in\mathcal T_K$.
...
\|\nabla w\|_{0,K}\lesssim h_K\|\div(\boldsymbol{\phi}-\boldsymbol{\phi}_1)\|_{0,K}\lesssim h_K\|\div\boldsymbol{\phi}\|_{0,K} +\sum_{F\in\XH{\mathcal F(K)}}h_F^{1/2}\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}.
\end{equation}
...
\|\boldsymbol{\phi}_2\|_{0,K}=\|I_K^{\div}(\nabla w)\|_{0,K}\lesssim \|\nabla w\|_{0,K}\lesssim h_K\|\div\boldsymbol{\phi}\|_{0,K} +\sum_{F\in\XH{\mathcal F(K)}}h_F^{1/2}\|\boldsymbol{\phi}\cdot\boldsymbol{n}\|_{0,F}.
\end{equation}
...
Based on DoFs \eqref{Vdivdof1}-\eqref{Vdivdof3}, take $\boldsymbol{\phi}\in\mathbb{V}_{k-1}^{\rm div}(K)$ such that
\begin{align*}
...
\end{align*}
Employing the norm equivalence \eqref{eq:Vkm1divnormequivalence}, %{eq:Vkm1divnormequiv},
we acquire
\begin{align*}
...
In this section we will develop a conforming VEM without extrinsic stabilization for the second order elliptic problem \eqref{eq:ellipitc2ndproblem} in arbitrary dimension.
% \begin{equation}\label{eq:ellipitc2ndproblem}
% \begin{cases}
% -\Delta u + \alpha u=f & \textrm{ in } \Omega,\\
% \qquad\qquad u=0&\textrm{ on } \partial\Omega,
...
\XH{Recall the $H^1$-conforming virtual element in \cite{ChenHuangWei2022,AhmadAlsaediBrezziMariniEtAl2013,BeiraoBrezziCangianiManziniEtAl2013,BeiraoBrezziMariniRusso2014}.
By assumption (A1), all the $(d-1)$-dimensional faces of $K$ are simplices, so we can require the shape function restricted to each face to be a polynomial when defining an $H^1$-conforming virtual element.
