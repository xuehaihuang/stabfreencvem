...
In short, the stabilization term has to be chosen carefully for different partial differential equations to make the VEM work well, which is arduous and \XH{might} reduce its practicality.
...
We intend to construct VEMs without extrinsic stabilization in arbitrary degree of polynomial, \XH{including a nonconforming VEM and a conforming VEM in arbitrary dimension},  in a unified way.
...
Since there is no additional stabilization term, these VEMs \XH{might be of interest for} the engineering community. More benefits of the VEMs without extrinsic stabilization will be the study of future works.
...
% Given a $d$-dimensional polytope $K$, let $\mathcal{F}(K)$ and $\mathcal{E}(K)$ be the set of all $(d-1)$-dimensional faces and $(d-2)$-dimensional faces of $K$ respectively. 
...
% , and all the $(d-1)$-dimensional faces of $K$ are also faces of $\mathcal T_K$.
\end{itemize}
}
