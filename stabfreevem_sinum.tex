...
\ifpdf
\hypersetup{
  pdftitle={Robust Mixed Methods for Quad-Curl Perturbation Problem},
  pdfauthor={X. Huang and C. Zhang}
}
\fi
...
\end{equation}
for some $q_{ij}\in\mathbb P_{k-2}(T)$. Here $\{\boldsymbol{N}_{ij}\}_{1\leq i<j\leq d}$ is the basis of antisymmetric matrice space $\mathbb K$ being dual to $\{\skw({\boldsymbol n_i\boldsymbol n_j^{\intercal}})\}_{1\leq i<j\leq d}$, i.e.,
\[
