...
as shown in Figure \ref{fig:mesh}. We use \eqref{eq:vem} and \eqref{eq:cfmvem} 
respectively to compute Example \ref{ex}, the numerical results for $k = 1, 2, 5$ are 
listed in Figure \ref{fig:rate1} \ref{fig:rate2}, 
...
of $H^1$-nonconforming virtual element method on 
...
\centering
\caption{Error $\|u - Q_h u_h\|_0$ and $\|\nabla u - Q_{h, k}^{div}\nabla u_h\|_0$ 
of $H^1$-conforming virtual element method  on $\mathcal T_0$(left) and 
